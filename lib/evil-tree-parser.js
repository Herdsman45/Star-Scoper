...
function parseEvilTreeTime(ocrText) {
  ...
  if (normalized.includes("ready")) {
    console.log(
      "[EVIL_TREE] Evil tree is ready now - setting to +93 minutes for next spawn",
    );
    // Set to 93 minutes from now to catch next tree (avoids nearly-dead trees at 0-35 mins past)
    return Date.now() + 93 * 60 * 1000;
  }
