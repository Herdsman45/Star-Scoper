 ...
 * @returns {object|null} - Object with {timestamp: number, estimated: boolean}, or null if parsing failed
 */
function parseEvilTreeTime(ocrText) {
  ...
  if (normalized.includes("ready")) {
    console.log(
      "[EVIL_TREE] Evil tree is ready now - setting to +128 minutes for next spawn",
    );
    // Set to 128 minutes from now to catch next tree (avoids nearly-dead trees at 0-35 mins past)
    return {
      timestamp: Date.now() + 128 * 60 * 1000,
      estimated: true,
    };
  }
