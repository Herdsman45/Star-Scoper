...
function extractWorldNumber(input) {
  ...
  // For "RuneScape59" or "RuneScape 59" style text - direct regex to extract
...
function extractRelativeTime(inputphrase) {
  ...
  if (directTimeMatch) {
    // We have a range like "33 to 35 minutes" - use the lower bound
    const firstNum = parseInt(directTimeMatch[1], 10);
    const secondNum = parseInt(directTimeMatch[2], 10);

    if (!isNaN(firstNum) && !isNaN(secondNum)) {
      return firstNum; // Use lower bound of the range
    }
  }
  ...
  if (time_text_startIndex < 4 || time_text_endIndex < 0) {
    return "Unknown"; // Default if we can't parse
  }
  ...
  if (isNaN(minutes_to_star) || minutes_to_star < 1) {
    return "Unknown"; // Default to Unknown if invalid
  }
