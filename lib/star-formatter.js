...
function extractWorldNumber(input) {
  ...
  const scapeMatch = correctedInput.match(/(R.{0,2}ne.?Sca.{0,2}pe?)([il\d\s]{1,5})/i);
  if (scapeMatch) {
    ...
    const cleanedNumber = scapeMatch[2]
      ...
      .replace(/l/gi, '1')
      .replace(/\s+/g, '');
    correctedInput = input.substring(0, scapeMatch.index) + 
                     scapeMatch[1] + cleanedNumber + 
...
function extractSize(inputphrase) {
  ...
  // This handles OCR errors like "Grof3e 3" or "Grosse 5"
  const germanSizePattern = /(?:größe|grosse|grösse|grof3e|grofte|grofse|gro3e|grobe|grore|groBe)\s*(\d+)/i;
...
function extractRelativeTime(inputphrase) {
  ...
  let norm = inputphrase
    ...
    .replace(/oto\s+(\d+)/gi, "0 to $1") // Fix "Oto 14" -> "0 to 14"
    .replace(/o\s*to\s+(\d+)/gi, "0 to $1") // Fix "O to 14" -> "0 to 14"
    .replace(/ +/g, " ")
    .replace(/to\s*\n\s*/gi, "to ")
    .replace(/\s{2,}/g, " ")
    .trim();
  ...
  // Example: "57 minutes a 1 heure et 21 minutes" -> should return 57, not 81
  // Must check BEFORE the hour+minute patterns to get the lower bound
  // Only match if this is truly standalone minutes (not "hour X minutes")
  ...
  if (minutesToHourMatch) {
    // Check if there's a "digit hour" pattern before this match
    // Example: "1 hour et 16 minutes" should not trigger this pattern
    const matchIndex = norm.indexOf(minutesToHourMatch[0]);
    const before = norm.substring(Math.max(0, matchIndex - 25), matchIndex).toLowerCase();
    // Look for "digit hour/heure/hora/stunde" pattern (hour comes AFTER digit)
  ...
  // German-specific pattern: "X Stunden und Y Minuten" (X hours and Y minutes)
  // Example: "2 Stunden und 1 Minuten bis 2 Stunden und 3 Minuten"
  const germanHourMinPattern = /(\d+)\s*(?:stunden?|hours?)\s*(?:und|and)\s*(\d+)\s*minutes?/i;
  const germanMatch = norm.match(germanHourMinPattern);
  if (germanMatch) {
    const hours = parseInt(germanMatch[1], 10);
    const minutes = parseInt(germanMatch[2], 10);
    if (!isNaN(hours) && !isNaN(minutes) && minutes < 60) {
      return hours * 60 + minutes;
    }
  }

  // French/Portuguese-specific pattern: "X heure/hora et/e Y minutes/minutos"
  // French: "1 heure et 44 minutes"
  // Portuguese: "1 hora e 36 minutos"
  const romanceHourMinPattern = /(\d+)\s*(?:heures?|horas?|hours?)\s*(?:et?|and)\s*(\d+)\s*minut\w*/i;
  const romanceMatch = norm.match(romanceHourMinPattern);
  if (romanceMatch) {
    const hours = parseInt(romanceMatch[1], 10);
    const minutes = parseInt(romanceMatch[2], 10);
    if (!isNaN(hours) && !isNaN(minutes) && minutes < 60) {
      return hours * 60 + minutes;
    }
  }
