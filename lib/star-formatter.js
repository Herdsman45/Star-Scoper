...
function extractSize(inputphrase) {
  ...
  const germanSizePattern =
    /(?:größe|grosse|grösse|grof3e|grol3e|grofte|grofse|gro3e|grobe|grore|groBe|grorre|grorRe|groRRe)\s*(\d+)/i;
...
function extractRelativeTime(inputphrase) {
  ...
  const minutesToHourPattern =
    /\b(\d+)\s*minut\w*\s*(?:a|to)\s*\d+\s*(?:heures?|horas?|hours?)/i;
  ...
  if (minutesToHourMatch) {
    ...
    const hasDigitHourBefore = /\d+\s*(?:hour|heure|hora|stunde)[s]?/i.test(
      before,
    );
  ...
  const germanHourMinPattern =
    /(\d+)\s*(?:stunden?|hours?)\s*(?:und|and)\s*(\d+)\s*minutes?/i;
  ...
  const romanceHourMinPattern =
    /(\d+)\s*(?:heures?|horas?|hours?)\s*(?:et?|and)\s*(\d+)\s*minut\w*/i;
